...
class Validator(object):
    ...
    def __init__(self, val_dataset, model, criterion, args):
        ...
        self.args = args
        self.logger = Logger(train=False)
        
        return

    def __call__(self, epoch):
        self.logger.new_epoch(epoch)
        for step, (feature, label) in enumerate(self.val_loader):
            try:
...
class Trainer(object):
    ...
    def __call__(self, epoch):
        """Train one epoch"""
        self.logger.new_epoch(epoch)
        for step, (feature, label) in enumerate(self.train_loader):
            try:
